...
def objective(trial):
    """
    Optimize the hyperparameters of a machine learning model using Optuna.

    Parameters:
        trial (optuna.Trial): An Optuna trial object that represents a single optimization trial.

    Returns:
        float: The mean absolute error (MAE) of the model's predictions on the test data.
    """
    ...
    if model_type in ['neural_network', 'gnn']:
        ...
        for epoch in range(100):  # Make this dynamic based on config
            model.train()
            optimizer.zero_grad()
...
def tune_hyperparameters():
    """
    Tune hyperparameters using Optuna library.
